...
const outcomeSelectWallet = async (interaction: StringSelectMenuInteraction) => {
    try {
        ...
        const categoryMenu = new StringSelectMenuBuilder()
            .setCustomId(`outcome_select_cat_${selectedWalletId}`)
        ...
        await interaction.update({
            content: "Great! Now, choose the category for this outcome:",
            components: [row]
        });
    } catch (error) {
        console.error(error);
...
export default outcomeSelectWallet