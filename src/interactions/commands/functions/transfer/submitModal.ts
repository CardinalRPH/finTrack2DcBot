...
const transferSubmitModal = async (interaction: ModalSubmitInteraction) => {
    try {
        await interaction.deferReply({ ephemeral: true });

        const [, , walletId, toWalletId] = interaction.customId.split("_");
        const amount = interaction.fields.getTextInputValue("transfer_amount");
        const description = interaction.fields.getTextInputValue("transfer_desc") || null;
        ...
        await interaction.editReply(`✅ **Transfer Successfully Saved!**\n\n💰 Amount: **\`${formatToRupiah(amount)}\`**\n📝 Notes: *${description || '-'}*`);
...
export default transferSubmitModal